<?php
/**
 * Property-Based Test for Notification Content Completeness
 * Feature: building-queue-system, Property 28: Notification Content Completeness
 * Validates: Requirements 10.2
 * 
 * Property: For any build completion notification, it should include the building name and new level.
 */
...
echo "=== Notification Content Completeness Property Test ===\n\n";
 ...
 */
PropertyTest::forAll(
    // Generator: Create random test scenario with different buildings and levels
    function() use ($conn) {
    ...
    // Property: Notification contains building name and level
    function($input) use ($conn, $buildingQueueManager) {
        // Get or create test user
        $stmt = $conn->prepare("SELECT id FROM users WHERE username = 'test_content_user' LIMIT 1");
        $stmt->execute();
        ...
        if (!$villageRow) {
            ...
            $villageName = 'Test Content Village';
            $x = rand(1, 100);
            $y = rand(1, 100);
        ...
        $stmt = $conn->prepare("SELECT id, name FROM building_types WHERE internal_name = ?");
        ...
        $stmt = $conn->prepare("SELECT message FROM notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT 1");
        $stmt->bind_param("i", $userId);
        $stmt->execute();
        $notificationRow = $stmt->get_result()->fetch_assoc();
        $stmt->close();
        ...
        // Check if message contains building name
        if (stripos($message, $buildingName) === false) {
            return "Notification message does not contain building name '{$buildingName}': {$message}";
        }
        
        // Check if message contains level
        if (stripos($message, (string)$input['level']) === false) {
            return "Notification message does not contain level '{$input['level']}': {$message}";
        }
