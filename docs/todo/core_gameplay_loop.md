...
- [x] Loop-specific tutorials/tooltips: surface context tips (empty queue, overcap resources, stale intel) with reason codes and quick actions. _(overview now shows nudges with reason codes + quick actions to town hall/barracks/warehouse/intel when queues idle, resources capped, or intel stale)_
