...
- **War Room Overlay:** In an active op, player toggles War overlay on map; sees operation markers and assigned targets. Selects a card in right panel to auto-center map and prefill rally preset; confirmation shows synchronized landing window and warns if outside target window. Ops chat snippet pinned in bottom bar.
- **Queued Actions Offline:** Player queues build/recruit/attack while online, goes offline; on return, queued items reflect success/failure states with reason codes; retry buttons appear inline.
